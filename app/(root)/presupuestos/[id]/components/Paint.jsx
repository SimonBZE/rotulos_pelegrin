...
export const Paint = ({
  ...
  useEffect(() => {
    ...
    const material = preciosServicios.pintura.material[formik.values?.pintura?.[index].material] || 0;    
    const metros_cuadrados = formik.values?.pintura?.[index].ancho * formik.values?.pintura?.[index].alto || 0;
    const lijado =  formik.values.pintura?.[index]?.lijado ? 50 : 0
    const newTotal = ((material * metros_cuadrados) + (metros_cuadrados * lijado)) * formik.values?.pintura?.[index].cantidad  || 0;
  ...
  return (
    <>
       ...
       {/* Precio */}
       <Pricing index={index} service={"pintura"} formik={formik} total={total} />
     </div>
    }
    </>
