...
export const ModalEditarCliente = ({
  ...
  const getClient = async () => {
    try {
      const { data } = await clientCtrl.getClienteTest(id);
      setCliente(data);
      formik.setValues({
    ...
    onSubmit: async (formData) => {
      try {
        ...
        if (actualizarCliente) {
          actualizarCliente(); // Invocar la función pasada para recargar los datos
        }
      } catch (error) {
        console.log(error);
        ...
        <form className="flex flex-col gap-3" onSubmit={formik.handleSubmit}>
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Select
              ...
              onChange={formik.handleChange}
              isInvalid={
                formik.touched.tipo && formik.errors.tipo ? true : false
              }
            >
